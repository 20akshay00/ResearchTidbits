...
[RK4](https://en.wikipedia.org/wiki/Runge%E2%80%93Kutta_methods) seems like a good place to start. We begin by defining a struct to hold the state of the integrator, $(u, tspan)$, where $u$ can be anything that implements the methods `similar` and supports basic algebraic operations, and tspan is an `AbstractRange` specifying the time interval of problem. The remaining variables are dummies to avoid allocation in a [hot loop](https://en.wikipedia.org/wiki/Hot_spot_(computer_programming)).
```{julia}
...
Then, we require only one generic function that loops through the time-steps. Below we just implement a basic version, but there is nothing stopping us from being more sophisticated with adaptive algorithms as well. At this point, we have introduced the notion of a callback as a mutating function that takes the input `(iter, integrator)::(Integer, AbstractIntegrator)` and is invoked at the end of every iteration. We see now that the callback can be utilized by the user to run custom logic within the integrator loop without ever having to touch the actual internals!
...
```
We can now solve any ordinary differential equation with the RK4 method!
```{julia}
	solve!(dfdt!, [5.], range(0., 1., length=100), solver = RK4Integrator, (callback!)=CallbackList([record_state, kick_callback]))
```

Note that this function only returns the value of `u` at the last time-step, which is a bit weird since typically we would want the evolution of the state at a time-series. While this was an artifical oversight on my part, it also provides a good opportunity to utilize callbacks to store custom data during the solver steps. 

# The callback struct
Generally, custom logic can be stateful (i.e, have persistent local data), one would need to create a closure over the function that actually performs the mutating action on the state of the integrator. However, Julia offers another (equivalent) alternative; namely, to define a struct that encapsulates the data, which can then be invoked as a function with access to this data. Since both structs and functions may be invoked by means of a function call syntax, I will generically refer to them as `callables` henceforth.
...
```
Now that the basic structure is in place, let us implement some common `condition`s. Again, these don't _have_ to be structs, but since the specific use-cases here require statefulness, they are an appropriate choice. The only restriction regarding the function call is that they must return a `bool` as mentioned earlier. We have not placed explicit safegaurds to statically check this, so the program would fail at run-time if the signature does not match what is expected.
```{julia}
begin
...
begin
    ...
    solve!(dfdt!, [5.0], range(0.0, 1.0, length=100), solver=RK4Integrator, (callback!)=record_state, kick_callback)
...
```
I think is quite a nice example of why the callback pattern allows modularity and extensibility for the user with no need to poke into the internals of the core solver loop. It should be noted that design patterns such as this one tend to quickly ramp up in complexity and the overhead introduced both in compilation time and developer maintanence time can often outweigh its usefulness. So its important to keep your specific use-case in mind and try to use them sparingly only when strictly required.