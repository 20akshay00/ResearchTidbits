...
# Why bother?
While at a first glance, this definition seems really abstract and not very useful, it actually opens up a lot of things to explore.
...
```
We now add a simple function to compute the second derivative of the energy under a phase twist using a finite-difference scheme. 
...
We then compare the phase diagram plotted using the order parameter $\langle \hat a \rangle$ (the first plot) and the superfluid density $\rho_s$ (the second plot).
...
```
We see that the superfluid density drops to zero exactly in the Mott insulator lobes!
