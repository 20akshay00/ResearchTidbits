...
function plot_phase_twist_response(t, mu; nmax = 5, twist_angles = range(-π/2, π/2, length=50))
    ...
    state, Egs = solve(â, n̂, t=t, mu=mu)
    ...
    plot!(xlabel = "Particle number", ylabel = "Coefficient", title = "\nGround state distribution", ylims = (0, 1.05))
...
let
	...
	heatmap(ts, mus, res)
    plot!(xlabel = "Hopping strength (t/U)", ylabel = "Chemical potential (μ/U)", colorbar_title="\nOrder parameter")
end
```
...
```
The superfluid density drops to zero exactly in the Mott insulator lobes! Further, we note that the transition is continuous as one would expect since the superfluid density should slowly fall off as the depth of the optical lattice is increased.
