...
# ╔═╡ 52da6f76-1fff-49f5-913a-29fef3881adf
using Plots, LinearAlgebra, LaTeXStrings

# ╔═╡ 4d873d6a-2a5d-11f0-09e3-d71371ceb9df
begin
	# â, n̂
	mean_field_ops(nmax) = (diagm(1 => sqrt.(1:nmax)), diagm(0 => 0:nmax))
 
	BHM(â, n̂; t, mu, psi, U = 1., z = 2) = -mu * n̂ + 0.5 * U * n̂ * (n̂ - I) - t * z * psi * (â + â') + t * z * psi^2 * I
	
	function solve(â, n̂, H; atol = 1e-8)
	...
	function superfluid_density(â, n̂, t, mu; atol = 1e-8, ϵ=1e-8)
		Htwist(ϕ) = psi -> BHM(â, n̂, t=t*cos(ϕ), mu=mu, psi=psi)
		function Etwist(ϕ)
			state, psi = solve(â, n̂, Htwist(ϕ), atol = atol)
			energy = state' * Htwist(ϕ)(psi) * state
		end

		return (Etwist(ϵ) - 2 * Etwist(0) + Etwist(-ϵ))/ϵ^2
	end
end

# ╔═╡ 956e7f55-cbb2-48ff-8341-38bbc55408ff
begin
...
# ╔═╡ 080efb2c-bc75-4299-b753-bb7d57e5991e
let
	â, n̂ = mean_field_ops(4)
	npoints = 50
	ts = range(0., 0.12, npoints)
	...
	Threads.@threads for idx in CartesianIndices(res)
		res[idx] = superfluid_density(â, n̂, ts[idx.I[2]], mus[idx.I[1]], ϵ=1e-6)
	end

	heatmap(ts, mus, res)
end

# ╔═╡ 10792ba4-d632-4796-9c20-4186c556b533
res[10, :] |> plot
...
manifest_format = "2.0"
project_hash = "bce8999ba40f0ea10555b7c88abad5471f463456"
