...
# ╔═╡ dbeefc52-404c-11f0-06d2-914bdbaa628e
using Plots, OrdinaryDiffEq, LinearAlgebra, DataStructures, StatsBase, Printf

# ╔═╡ 761e3b7f-dc5e-45d0-aa9f-52ab0360d48f
begin
	function construct_hamiltonian(x, V, boundary_condition = :OBC)
		N, Δx = length(x), step(x)
		H = (-1 / (2 * Δx^2)) * SymTridiagonal(-2. * ones(N), ones(N-1)) + Diagonal(V.(x))
...
# ╔═╡ c9e47f7c-8fca-4673-b73c-ccfe1eeb8ef6
begin
	mutable struct ConvergenceCallback
		measure
		val
		window::Int
		atol::Float64

		ConvergenceCallback(measure, window, atol) = new(measure, CircularBuffer{Float64}(window), window, atol)
	end

	function (p::ConvergenceCallback)(integrator)
		push!(p.val, p.measure(integrator))

		if length(p.val) == p.window && std(p.val) < p.atol
			terminate!(integrator)
		end
	end
end

# ╔═╡ b61fb520-32ee-4fe2-8a46-77c5f66d4f0a
function generate_evolution_gif(sol, exact_sol, H, x, V; num_points = 100, energylogscale = true, exact_sol_label = "Exact")
	Δx = step(x)

	prob_amplitude_exact, energy_exact = exact_sol
	ymax = 2.5 * maximum(abs2, prob_amplitude_exact)

	ts = range(sol.t[1], sol.t[end], num_points)
	prob_amplitudes = map(psi -> psi ./ sqrt(sum(abs2, psi) * Δx), sol.(ts))
	energies = map(psi -> dot(psi, H(psi)) * Δx, prob_amplitudes)
	energy_errs = abs.(energies .- energy_exact) ./ abs.(energy_exact)

	@gif for idx in eachindex(ts)
		# imaginary time probability density
		p1 = plot(legend=:topright, framestyle=:box, ylims = [0., ymax], ylab = "Probability density", xlab = "Position", title = "\nWave function")
		plot!(x, abs2.(prob_amplitudes[idx]), lab = "t=$(@sprintf("%.3f", ts[idx]))", c = 1)
		plot!(x, abs2.(prob_amplitude_exact), lab = exact_sol_label, c = :black)
	
		# potential
		plot!(twinx(), x, V.(x), c = :black, ls = :dash, ylab = "Potential", lab = "Potential", legend=:topleft)

		# imaginary time energy
		if energylogscale
			p2 = plot(legend=:topright, framestyle=:box, title = "\nEnergy", xlab = "Imaginary time", ylab = "|E - E₀|/|E₀|", xlims = (first(ts), last(ts)), ylims = [1e-14, 1e2], yscale=:log10)
			plot!(ts, energy_errs, lab = "", lw = 1., c = :black, ls = :dash)
			scatter!([ts[idx]], [energy_errs[idx]], c = 1, ms = 3, lab = "")
		else
			p2 = plot(legend=:topright, framestyle=:box, title = "\nEnergy", xlab = "Imaginary time", ylab = "E", xlims = (first(ts), last(ts)), ylims = [0, maximum(energies) * 1.05])
			hline!([energy_exact], c = :black, ls = :dash, lw = 1, lab = exact_sol_label)
			plot!(ts[1:idx], energies[1:idx], lab = "", lw = 2, c = 1)
		end
		
		plot(p1, p2, layout = (1, 2), size = (900, 300), margins = 10Plots.mm, leftmargin=12Plots.mm)
	end
end

# ╔═╡ b7f580d1-e023-4fee-83cd-c38a4a30d05e
begin
	xmax = 5.
	x = range(-xmax, xmax, length = 500)
	V(x) = 0.0 * x^4 + 0.5 * x^2 
	const H = construct_hamiltonian(x, V, :OBC)
end;

# ╔═╡ ebf5836f-9d96-4644-8c70-d50a2c141877
begin
	function imaginary_time_evolution(O, x; tmax = 100., atol = 1e-4, window = 100, save_history = true, projection_vectors = nothing, num_particles = 1, alg = Tsit5())

		# derivative
		dΨdt!(dΨ, Ψ, (H, x), t) = (dΨ .= -1. * O(Ψ))
		
		# force normalize every step
		normalize_callback = DiscreteCallback(
			(u, t, integrator) -> true, # condition
			(integrator) -> ( # affect!
				integrator.u ./= sqrt(sum(abs2, integrator.u) * step(integrator.p[2])) * num_particles
			),
			save_positions=(false, false)
		)
	
		# remove component along projection_vector
		projection_callback = DiscreteCallback(
			(u, t, integrator) -> true, # condition
			(integrator) -> (integrator.u .-= step(integrator.p[2]) * sum(dot(state, integrator.u) * state for state in projection_vectors)), 
			save_positions=(false, false)
		)
	
		# check convergence using ||HΨ|| as a measure
		convergence_callback = DiscreteCallback(
			(u, t, integrator) -> true, # condition
			ConvergenceCallback((integrator) -> dot(integrator.u, O(integrator.u))/dot(integrator.u, integrator.u), window, atol), 
			save_positions=(false, true)
		)
	
		if isnothing(projection_vectors)
			callback = CallbackSet(normalize_callback, convergence_callback)
		else 
			callback = CallbackSet(normalize_callback, projection_callback, convergence_callback)
		end
		
		prob = ODEProblem(dΨdt!, ones(length(x)), (0., tmax), (H, x), callback = callback)
		sol = solve(prob, alg)
	
		return save_history ? sol : sol.u[end]
	end
	
	function get_eigenstate(O, x, n; tmax = 100., atol = 1e-4, window = 100, num_particles = 1)
	
		eigenstates = Vector{Vector{Float64}}(undef, n)
		eigenstates[1] = imaginary_time_evolution(O, x, tmax=tmax, atol=atol, window=window, save_history=false, num_particles=num_particles) # ground state
	
		for i in 2:n
			eigenstates[i] = imaginary_time_evolution(O, x, tmax=tmax, atol=atol, window=window, projection_vectors = eigenstates[1:i-1], save_history=false, num_particles=num_particles)
		end
...
# ╔═╡ 9fe9038f-1a0e-4694-a64e-b90241a36b77
begin
	function exact_tise(H, x)
		vals, vecs = eigen(H)
		return vecs[:, 1] / sqrt(sum(abs2, vecs[:, 1]) * step(x)), vals[1]
	end
	
	function exact_thomas_fermi(V, g, xs; N=1.0, atol=1e-6, maxiter=100)
	    dx = step(xs)
	    f(mu) = sum(((mu .- V.(xs)) ./ g) .* (mu .> V.(xs))) * dx - N
	    mu_lo, mu_hi = minimum(V.(xs)) + 1e-6, maximum(V.(xs)) + 10.0
		mu = (mu_lo + mu_hi) / 2
	    for _ in 1:maxiter
	        mu = (mu_lo + mu_hi) / 2
	        f(mu) > 0 ? (mu_hi = mu) : (mu_lo = mu)
	        abs(f(mu)) < atol && break
	    end
	    density = ((mu .- V.(xs)) ./ g) .* (mu .> V.(xs))
	    return sqrt.(density), mu
	end
end

# ╔═╡ 1cf9322c-56ef-460d-8af4-2f0a58b25b1b
let
	O = ψ -> H * ψ
	sol = imaginary_time_evolution(O, x, atol=1e-8, num_particles = 1)
	exact_sol = exact_tise(H, x)
	generate_evolution_gif(sol, exact_sol, O, x, V)
end

# ╔═╡ 75334ce3-3783-4e76-9f9e-87bc4a6c2319
let
	g = 20
	O = ψ -> H * ψ .+ (g .* abs2.(ψ) .* ψ)
	sol = imaginary_time_evolution(O, x, atol=1e-7, num_particles = 1)
	exact_sol = exact_thomas_fermi(V, g, x)
	generate_evolution_gif(sol, exact_sol, O, x, V, energylogscale=false, exact_sol_label = "Thomas-Fermi")
end

# ╔═╡ 9e84185b-09eb-45a6-8aec-aa5e78e97d91
let
	states = get_eigenstate(ψ -> H * ψ, x, 4)
	p = plot()
	for (idx, state) in enumerate(states)
		plot!(x, idx .+ abs2.(state))
	end
	p
end
...
manifest_format = "2.0"
project_hash = "c93487da049150aaabab0c309319d55f48fbdcd7"
